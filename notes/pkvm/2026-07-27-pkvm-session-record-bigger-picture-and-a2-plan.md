...
- **`e09e26bea`** — `docs:` evidence bundle (5 files) under `docs/superpowers/specs/evidence/gen-coverage-diagnosis-2026-07-27/`.
...
Files likely touched (a2 impl, later): `sys/linux/dev_kvm_arm64.txt`, `executor/common_kvm_arm64.h` (the `syz_kvm_run_fw_fault_gen` helper), a new `sys/linux/test/` case. Reuse the existing ring-arm / `stats_reset` / `addr2line` tooling already in-tree: `docs/superpowers/specs/campaign-runbook.sh` and the `symbolize.sh` scripts under `docs/superpowers/specs/evidence/composite-campaign-2026-07-21/` and `campaign-2026-07-21/`.
