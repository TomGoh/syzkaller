...
- `notes/pkvm/2026-07-20-pkvm-syzkaller-support-design.md` — design spec; **this file**; and `notes/pkvm/evidence/2026-07-21-*` (phaseB, BUG1 report+repro, controlled-run md + RAW outputs, pkvm-call1 coverage).
...
Branch split: the code + tests + this `notes/pkvm/` tree (design spec, this record, `evidence/*`, `campaign-runbook.sh`) live on **both** branches; the narrative **`notes/*`** (the Chinese learning notes) stay **only** on the working branch `claude/syzkaller-source-learning-vvisp3` — that is the "learning notes left behind." The **kernel repo** (`/home/jose/common`) changes are **kept local, uncommitted** by intent (fuzzing config + toolchain pin + the `pr_debug` debugfs-warning quiet; the BUG1 fix deliberately not applied). Both manager configs and the `workdir*/` trees are gitignored (local-only).
